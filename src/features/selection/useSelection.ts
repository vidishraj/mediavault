 ...
 *
 * Callbacks are referentially stable (empty-dep useCallback), so spreading them
 * into grid/card props never invalidates a memoised card; a selection change
 * re-renders only via the `selectedIds` value a card actually reads.
 */
